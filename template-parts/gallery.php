<?php
/**
 * 6. 매장 분위기 — CONTENT_PLAN.md 3-6절. 손님 얼굴이 나온 사진은 동의 확인 전까지
 * 절대 사용하지 않습니다 (00_READ_FIRST.md 작업 규칙). 지금은 안전하게 확보된
 * 사진이 없어 플레이스홀더로 대체합니다.
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
?>
<section class="section">
	<div class="container">
		<span class="section-eyebrow">매장 분위기</span>
		<h2>가족, 지인과 함께 편하게</h2>
		<p class="section-desc">부모님 모시고 오기 좋은 공간입니다. (좌석/접근성 정보 확정 후 문구 보강 예정)</p>

		<div class="gallery-grid">
			<div class="gallery-grid__item"><img src="<?php echo esc_url( get_template_directory_uri() . '/assets/photos_web/gallery-01.jpg' ); ?>" alt="매장 내부 (사진 내용 확인 전 임시 배치)"></div>
			<div class="gallery-grid__item gallery-grid__item--placeholder">매장 사진<br>준비 중</div>
			<div class="gallery-grid__item gallery-grid__item--placeholder">매장 사진<br>준비 중</div>
		</div>
		<div class="menu-note">
			손님 얼굴이 나온 사진은 사용 동의 확인 전까지 게시하지 않습니다. 매장 사진은 확정되는 대로
			<code>assets/photos_web/</code>에 추가해 이 섹션을 채웁니다.
		</div>
	</div>
</section>
